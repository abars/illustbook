...
class AddNewThread(webapp.RequestHandler):
	...
	def post(self):
		...
		if(SpamCheck.check(self.request.get('thread_title'),checkcode) or SpamCheck.check(self.request.get('comment'),checkcode) or SpamCheck.is_spam_ip(self.request.remote_addr,user)):
			if(is_english):
